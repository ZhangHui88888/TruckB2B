...
| 功能 | 状态 | 说明 |
|------|------|------|
| 产品表 products | ✅ 完成 | 产品信息存储 |
| 品牌表 brands | ✅ 完成 | 8 个卡车品牌 |
| 分类表 categories | ✅ 完成 | 产品分类 |
| 产品视图 products_view | ✅ 完成 | 关联查询视图 |
| 搜索函数 search_products | ✅ 完成 | 全文搜索 |
| SEO 相关表 | ✅ 完成 | page_seo, keyword_performance 等 |
...
| Schema.org | ✅ 完成 | Organization/Product/FAQ |
...
| 任务 | 说明 | 预计工时 |
|------|------|----------|
| **Cloudflare Pages 环境变量** | 配置 Supabase URL/Key，使网站能读取产品数据 | 10 分钟 |
| **Supabase 知识库表** | 创建 knowledge_base 表（RAG 用） | 10 分钟 |
| **Cloudflare Worker 后端** | AI 客服 API、询盘处理、邮件通知 | 4-6 小时 |
| **AI 客服对话功能** | 前端对接 Worker API，实现真实对话 | 2-3 小时 |
| **询盘表单功能** | 表单提交、邮件通知、数据存储 | 2-3 小时 |
| **知识库导入** | 产品知识向量化，存入 Supabase | 2-3 小时 |

### 🟡 中优先级（增强功能）

| 任务 | 说明 | 预计工时 |
|------|------|----------|
| **其他品牌产品同步** | 同步 SCANIA/MAN/IVECO 等品牌产品到 Supabase | 1-2 小时 |
| WhatsApp Business API | Meta 开发者验证、Webhook 配置 | 3-4 小时 |
| 后台管理功能对接 | 连接真实数据，实现 CRUD | 4-6 小时 |
| Google Analytics 4 | 流量统计分析 | 1 小时 |
| Bing Webmaster | 搜索引擎提交 | 0.5 小时 |

### 🟢 低优先级（可选优化）

| 任务 | 说明 | 预计工时 |
|------|------|----------|
| **博客/新闻功能** | 静态 Markdown 博客，提升 SEO 内容更新频率 | 2-3 小时 |
...
|------|------|
| `部署准备清单.md` | 账号密钥、配置步骤 |
| `外贸独立站技术方案.md` | 技术架构、代码示例 |
| `Cloudflare-Pages部署指南.md` | 前端部署步骤 |
| `Supabase配置指南.md` | 数据库配置 |
| `Google-Cloud-GSC配置指南.md` | GSC API 配置 |
| `SEO配置记录.md` | SEO 配置状态 |
| `对话提示词.md` | AI 客服 Prompt |
| `账号密钥汇总.md` | 密钥快速查阅 |
